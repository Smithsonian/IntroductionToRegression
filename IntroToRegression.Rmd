...
As we've done previously, we will 'clean up' our Workspace when starting a new session. We will also make sure the dplyr package is loaded for doing data manipulation.
...
Note, all columns/variables included in this sample dataset are numeric-type. You can confirm the data type of each column by typing **is.numeric()** next to the variable name (e.g., `is.numeric(Grazing_Data$GrassHt)`). 
...
You could plot any of the variables in the dataframe. Plotting the data is one of the simplest ways to look at and explore your data.

In **R** you can learn lots of ways to customize your plots. But for now, let's change the 'x' and 'y' variable names and add a title to this plot.
...
Make a plot to show the relationship between cattle stocking rate and grazing height
...
Now, we will run several basic linear regression models that attempt to explain how grazing pressure (grazing height) affects various dependent variables, like cattle weight gain and vegetation biomass accumulation rates.  Finally, we will assign the model output to an object name "mod1". Remember to specify the `Grazing_Data` dataset in the model so that **R** understands that the name of dependent and independent variables are columns in that specific dataframe.

```{r}
# names(Grazing_Data)  # We may want to print the variable names in our dataset before fitting models
lm1 <- lm(HerbageMass ~ GrazingHt, data = Grazing_Data)
```

### Investigate your model

Once you have fit the model, you can use the summary function to investigate the beta coefficients, SEs, and intercept value. 
...
We don't want values beyond 1. Our 12th observation has a Cook's distance close to 1, but generally we aren't too concerned about outlier effects.
...
plot(Grazing_Data$HerbageAccumulationRate ~ Grazing_Data$GrazingHt,
     ...
     main="Relationship between Grazing Height and Pasture Growth")
```
...
What can we conclude? The best model (from the two we have tried) for predicting herbage accumulation rate assumes a non-linear (quadratic) relationship is best (`lm3`). In plain language, what we are seeing is 1) that pasture that is grazed short grows at a slower rate than pasture that is grazed higher, and 2) that accumulation rate levels off at values between 20-25cm.

To finish, let's make a plot with o
...
plot(Grazing_Data$GrazingHt, Grazing_Data$HerbageAccumulationRate, xlab="x", ylab="y", main="Regression", cex=2, pch=19)
